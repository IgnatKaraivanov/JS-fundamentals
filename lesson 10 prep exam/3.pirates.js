function pirates(input) {
  ...
  while (command !== "Sail") {
    ...
    if (!citiesMap.hasOwnProperty(town)) {
      // if there is not such city in the map, add it
      citiesMap[town] = { population, gold };
    } else {
      // if there is, update population and gold
  ...
  while (command !== "End") {
    ...
    if (townCommands[0] === "Plunder") {
      ...
      if (citiesMap[town].population <= 0 || citiesMap[town].gold <= 0) {
        // if there is no population or gold destroy the city
      ...
      if (gold < 0) {
        ...
        citiesMap[town].gold += gold;
        console.log(
          `${gold} gold added to the city treasury. ${town} now has ${citiesMap[town].gold} gold.`
        );
      }
    }
    command = input.shift(); //
  ...
  for (const kvp of sorted) {
    console.log(
      `${kvp[0]} -> Population: ${kvp[1].population} citizens, Gold: ${kvp[1].gold} kg`
    );
  }
}
